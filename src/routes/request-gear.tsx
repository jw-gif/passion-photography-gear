...
function RequestPage() {
  ...
  // We only care about pending or approved requests within ±60 days of today.
  useEffect(() => {
    void (async () => {
  ...
  // as a one-tap "recently requested" row.
  useEffect(() => {
      ...
      {selectedGear.size > 0 && (
        <div className="lg:hidden fixed bottom-0 inset-x-0 bg-card border-t border-border px-4 py-3 z-30">
          <div className="max-w-6xl mx-auto flex items-center justify-between gap-3">
            ...
            <Button type="button" size="sm" onClick={() => window.scrollTo({ top: document.body.scrollHeight, behavior: "smooth" })}>
              Review &amp; submit
            </Button>
          </div>
        </div>
