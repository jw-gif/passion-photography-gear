...
export const Route = createFileRoute("/admin_/events")({
  head: () => ({ meta: [{ title: "Events · Admin" }, { name: "robots", content: "noindex, nofollow" }] }),
  component: () => {
    const { signOut } = useAuth();
    return (
      <RequireAdmin requireAdmin>
        <main className="min-h-screen">
          <HubHeader onLogout={() => signOut()} title="Events" subtitle="Workshops & gatherings" />
          <EventsAdmin />
        </main>
      </RequireAdmin>
    );
...
interface RsvpCount { event_id: string; n: number; }
