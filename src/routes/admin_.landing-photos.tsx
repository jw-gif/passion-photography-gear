...
export const Route = createFileRoute("/admin_/landing-photos")({
  head: () => ({ meta: [{ title: "Landing photos · Admin" }, { name: "robots", content: "noindex, nofollow" }] }),
  component: () => {
    const { signOut } = useAuth();
    return (
      <RequireAdmin requireAdmin>
        <main className="min-h-screen">
          <HubHeader onLogout={() => signOut()} title="Landing photos" subtitle="Sign-in page gallery" />
          <LandingAdmin />
        </main>
      </RequireAdmin>
    );
