...
import {
  Camera, Wrench, Calendar, PlayCircle, Megaphone, LogOut,
  Briefcase, Check, Play, ArrowLeft,
} from "lucide-react";
...
function PhotographerPage() {
  ...
  const { user, loading, signOut, isTeam, isAdmin } = useAuth();
  ...
  const isOwner = !!photographer && !!user && photographer.user_id === user.id;
  const canView = isOwner || isTeam || isAdmin;

  useEffect(() => {
    if (loading) return;
    if (!user) {
      navigate({ to: "/login", search: { redirect: `/photographers/${id}` }, replace: true });
    }
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await supabase
        .from("photographers")
        .select("id, user_id, name, email, tier, token")
        .eq("id", id)
        .maybeSingle();
      setPhotographer((data ?? null) as Photographer | null);
      setAccessChecked(true);
    })();
  }, [id, user]);

  useEffect(() => {
    if (!photographer || !canView) return;
    (async () => {
      const [{ data: open }, { data: mine }] = await Promise.all([
        supabase.rpc("list_open_jobs", { _token: photographer.token }),
        supabase.rpc("list_my_jobs", { _token: photographer.token }),
      ]);
      ...
      if (evRows.length && photographer.user_id) {
        const ids = evRows.map((e) => e.id);
        const [{ data: mineR }, { data: all }] = await Promise.all([
          supabase.from("event_rsvps").select("event_id").in("event_id", ids).eq("user_id", photographer.user_id),
  ...
  async function toggleRsvp(eventId: string) {
    ...
    if (rsvps[eventId]) {
      ...
      const { error } = await supabase.from("event_rsvps").insert({
        event_id: eventId, user_id: user.id, photographer_id: photographer?.id, status: "going",
      });
      if (error) { toast.error(error.message); return; }
  ...
  if (!photographer) {
    return (
      <main className="min-h-screen flex items-center justify-center">
        <Card className="p-6 text-sm text-muted-foreground">Photographer not found.</Card>
      </main>
    );
  }
        ...
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <div className="font-semibold tracking-tight leading-tight truncate">{photographer.name}</div>
              <div className="text-xs text-muted-foreground truncate">
                {photographer.email} · <span className="capitalize">{photographer.tier.replace(/_/g, " ")}</span>
              </div>
            </div>
          </div>
            ...
            {isTeam && !isOwner && (
              <Button asChild variant="outline" size="sm">
                <Link to="/admin/team" search={{ tab: "photographers" }}>
                  <ArrowLeft className="size-4" /> All photographers
                </Link>
              </Button>
            )}
            {isOwner && (
      ...
      <div className="max-w-6xl mx-auto px-6 py-6 space-y-6">
        {isTeam && !isOwner && (
          <Card className="p-3 text-xs text-muted-foreground bg-muted/30">
            Admin view — you're seeing this photographer's personal page. RSVPs and actions are read-only here.
          </Card>
        )}
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Section title="Accepted jobs" icon={<Camera className="size-4" />}>
          ...
          <Section title="Gear requests" icon={<Wrench className="size-4" />}>
